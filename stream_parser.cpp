...
std::string to_utf8(const std::string& input) {
    ...
    error = U_ZERO_ERROR;
    ucnv_toUChars(conv, &u16str[0], size, input.c_str(), input.size(), &error);
    if (U_FAILURE(error)) {
        ucnv_close(conv);
        throw std::runtime_error("Failed to convert string to UTF-16");
    }

    size = ucnv_fromUChars(conv, nullptr, 0, u16str.c_str(), u16str.size(), &error);
    if (error != U_BUFFER_OVERFLOW_ERROR) {
        ucnv_close(conv);
    ...
    error = U_ZERO_ERROR;
    ucnv_fromUChars(conv, &output[0], size, u16str.c_str(), u16str.size(), &error);
    if (U_FAILURE(error)) {
        ucnv_close(conv);
...
void stream_parser::dump_http_request() {
    ...
    if (!output_json.empty()) {
        ...
        std::__fs::filesystem::path dir = std::__fs::filesystem::path(save_filename).parent_path();
